.DEFAULT_GOAL := all
.PHONY: all clean clean-tools clean-version deploy-fwbuild deploy-mos-binary deps downloads fwbuild-instance fwbuild-manager generate install linux mac mos version win
...
GOBINPATH := $(shell go env GOBIN)
export PATH := $(GOBINPATH):$(PATH)
SHELL := bash

all: clean-bin mos fwbuild-manager fwbuild-instance
...
deps:
	$(GOBINPATH)/govendor -version 2>/dev/null || \
		go install github.com/kardianos/govendor
	cd $(GOPATH)/src/github.com/mongoose-os/mos && $(GOBINPATH)/govendor sync -v
...
build-%: deps version generate-%
	go version
	GOOS=$(GOBUILD_GOOS) GOARCH=$(GOBUILD_GOARCH) CC=$(GOBUILD_CC) CXX=$(GOBUILD_CXX) \
	  go build -v -tags $(GOBUILD_TAGS) -ldflags $(GOBUILD_LDFLAGS) -o $(OUT) $(PKG)
